...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getTrainingGroups() {
    const data = await prisma.training_group
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'training_group'));
  ...
  async function createTrainingGroup() {
    ...
    const data = await prisma.training_group.create({
      data: body,
    });
    return res.status(200).json(data);
  }
