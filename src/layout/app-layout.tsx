...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['club-owner', 'coach', 'athlete', 'club-manager', 'venue-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ClubInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/clubs` : null),
    () => getClubs({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Club',
      path: isTenantUser ? `/clubs/view/${data?.[0]?.id}` : '/clubs',
      entity: 'club',
      service: AccessServiceEnum.PROJECT,
      icon: FiMapPin,
    },
    { name: 'Athlete', path: '/athletes', entity: 'athlete', service: AccessServiceEnum.PROJECT, icon: FiUserX },
    ...
    { name: 'Venue', path: '/venues', entity: 'venue', service: AccessServiceEnum.PROJECT, icon: FiMap },
